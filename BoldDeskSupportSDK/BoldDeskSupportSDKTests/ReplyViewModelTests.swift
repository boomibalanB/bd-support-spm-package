...
final class ReplyViewModelTests: XCTestCase {
    ...
    @MainActor func testValidateReplyText_emptyReturnsFalse() {
        let vm = ReplyViewModel(ticketId: 1, statusId: 1)
        vm.replyText = ""
        XCTAssertFalse(vm.validateReplyText(showToast: false))
    }

    @MainActor func testValidateReplyText_whitespaceReturnsFalse() {
        let vm = ReplyViewModel(ticketId: 1, statusId: 1)
        vm.replyText = "   \n\t "
        XCTAssertFalse(vm.validateReplyText(showToast: false))
    }

    @MainActor func testValidateReplyText_nonEmptyReturnsTrue() {
        let vm = ReplyViewModel(ticketId: 1, statusId: 1)
        vm.replyText = "Hello"
        XCTAssertTrue(vm.validateReplyText(showToast: false))
    }

    @MainActor func testValidateAndUpdateReply_doesNotProceedWhenInvalid() async {
        let vm = await ReplyViewModel(ticketId: 1, statusId: 1)
